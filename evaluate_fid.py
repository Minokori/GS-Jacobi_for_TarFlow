...
def main(args):
    ...
    model = GS_Jacobi_sampling.Model(
    ...
    for i in range(num_batches):
        ...
        while True:
            with torch.inference_mode(), torch.autocast(device_type='cuda', dtype=torch.bfloat16):
                print(args.num_GS_list)
                print(args.max_jacobi_list)
                samples = model.reverse(noise, y, args.cfg, attn_temp=args.attn_temp, annealed_guidance=True,
                                        num_GS_list=args.num_GS_list,max_jacobi_list=args.max_jacobi_list,guess_list=args.guess_list)
        ...
        if rank == 0:
            print(f"Updating progress bar: {i+1}/{num_batches}")
            pbar.update(1)
    ...
    if dist.local_rank == 0:
        tv.utils.save_image(samples, sample_dir / f'samples_{args.sample_name}_cfg{args.cfg:.2f}.png', normalize=True, nrow=16)
    dist.barrier()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--logdir', default='runs_GSJ', type=pathlib.Path, help='Path for artifacts')
    ...
    parser.add_argument('--num_GS_list',default=[], type=ast.literal_eval, help='the num of GS for each flow blocks, 0 for pure iteration')
    parser.add_argument('--max_jacobi_list', default=[], type=ast.literal_eval, help='the max num of iteration for each flow blocks')
    parser.add_argument('--guess_list', default=[], type=ast.literal_eval, help='initialize x with zero or z')
    ...
    parser.add_argument('--sample_name',default='original',type=str,help='the name of png sampled by model')
